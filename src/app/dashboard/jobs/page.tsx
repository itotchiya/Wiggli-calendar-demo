  ...
  return (
    <>
      ...
      <main className="jobs-page">
        <JobsPageHeader
          activeTab="Jobs"
          actions={<button className="create-job-button" type="button"><Plus size={16} /> Create a job</button>}
        />
          ...
          <table className="jobs-table">
            ...
            <tbody>
              {jobs.map((job) => {
                ...
                return (
                  <tr key={job.reference} onClick={() => router.push(`/dashboard/jobs/${job.reference}`)}>
                    <td className="jobs-table-menu-col" onClick={(event) => event.stopPropagation()} style={{ width: 72, minWidth: 72, maxWidth: 72 }}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><input type="checkbox" aria-label="Select row" style={{ width: 16, height: 16, accentColor: "#0f766e" }} /><button aria-label={`Actions for ${job.title}`} type="button" style={{ border: 0, background: "transparent", cursor: "pointer", color: "var(--muted)", display: "grid", placeItems: "center", padding: 2, width: 24, height: 24 }}><MoreHorizontal size={16} /></button></div>
                    </td>
