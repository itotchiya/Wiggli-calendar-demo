...
import { ChevronDown, MoreHorizontal, Video, MapPin, Building2, Link2, ExternalLink, Copy, Check, Target, BriefcaseBusiness, Users, Search, ListFilter, Columns3, Download, CalendarPlus, Plus } from "lucide-react";
...
type MeetingRow = {
  ...
  title: string;
  date: string;
  status: "Draft" | "Scheduled" | "Overdue" | "Done" | "Canceled" | "Error";
  meetingType: string;
...
const mockMeetings: MeetingRow[] = [
  {
    ref: "MT-179702", title: "Random Candidate Interview", date: "14 Apr 2026, 09:15 - 09:45", status: "Draft", meetingType: "Candidate meeting",
    organizer: { name: "Maya Thornton", avatar: "/avatars/avatar-1.webp", color: "#667eea" },
    linkedCandidate: { id: "ethan-patel", name: "Ethan Patel", avatar: "/avatars/avatar-2.webp" },
    linkedJob: { id: "development", title: "New development team" },
  ...
  {
    ref: "MT-179703", title: "ICT Meeting", date: "03 Nov 2024, 14:00 - 14:30", status: "Scheduled", meetingType: "Screening call",
  ...
  {
    ref: "MT-179705", title: "Marketing Discussion", date: "09 Jan 2026, 11:30 - 12:00", status: "Done", meetingType: "Client meeting",
    organizer: { name: "Maya Thornton", avatar: "/avatars/avatar-1.webp", color: "#667eea" },
    linkedCandidate: { id: "ethan-patel", name: "Ethan Patel", avatar: "/avatars/avatar-2.webp" },
    linkedContact: { id: "james-whitaker", name: "James Whitaker", avatar: "/avatars/avatar-4.webp" },
    linkedJob: { id: "development", title: "New development team" },
    linkedOrganization: { id: "gm", name: "General Motors", initials: "GM", color: "#dc2626" },
    attendees: [{ name: "Maya Thornton", avatar: "/avatars/avatar-1.webp" }, { name: "Ethan Patel", avatar: "/avatars/avatar-2.webp" }, { name: "James Whitaker" }, { name: "Liam Harper", avatar: "/avatars/avatar-5.webp" }, { name: "Sofia Morales", avatar: "/avatars/avatar-6.webp" }],
  ...
  {
    ref: "MT-179707", title: "Marketing Meeting", date: "05 Dec 2025, 13:20 - 13:50", status: "Error", meetingType: "Interview",
    organizer: { name: "Maya Thornton", avatar: "/avatars/avatar-1.webp", color: "#667eea" },
    linkedContact: { id: "james-whitaker", name: "James Whitaker", avatar: "/avatars/avatar-4.webp" },
    linkedOpportunity: { id: "2401", title: "2401 - VP of Global Equity" },
    linkedOrganization: { id: "gm", name: "Cheverolet", initials: "CH", color: "#38bdf8", parent: { name: "General Motors", initials: "GM", color: "#dc2626" } },
...
function StatusPill({ status }: { status: MeetingRow["status"] }) {
  const styles: Record<string, React.CSSProperties> = {
    Draft: { border: "1px solid #cbd5e1", color: "#64748b", background: "#fff" },
    Scheduled: { border: "1px solid #3b82f6", color: "#2563eb", background: "#eff6ff" },
    Overdue: { border: "1px solid #fb923c", color: "#c2410c", background: "#fff7ed" },
    Done: { border: "1px solid #86efac", color: "#15803d", background: "#f0fdf4" },
    Canceled: { border: "1px solid #fca5a5", color: "#dc2626", background: "#fef2f2" },
    Error: { border: "1px solid #fca5a5", color: "#b91c1c", background: "#fef2f2" },
  };
...
function MeetingPlaceIcon({ provider }: { provider?: string }) {
  ...
  if (provider === "company") return null;
  return null;
}

function buildMeetingPreview(row: MeetingRow): EventItem {
  // parse "14 Apr 2026, 09:15 - 09:45"
  let year = "2026", month = "04", day = "14", hour = 9, minute = 15, endHour = 9, endMinute = 45;
  try {
    const parts = row.date.split(", ");
    if (parts.length === 2) {
      const dPart = parts[0].trim(); // "14 Apr 2026"
      const tPart = parts[1].trim(); // "09:15 - 09:45"
      const dTokens = dPart.split(" ");
      if (dTokens.length === 3) {
  ...
  const numericId = Number(String(row.ref).replace(/\D/g, "")) || Math.floor(Math.random()*900000+100000);
  const statusToAttendee: Record<string, "Pending"|"Accepted"|"Declined"|"Tentative"> = { Draft:"Pending", Scheduled:"Pending", Overdue:"Pending", Done:"Accepted", Canceled:"Declined", Error:"Tentative" } as any;
  ...
  const avatarFor = (name: string, existing?: string) => {
    ...
    // deterministic fallback to local avatars 1-24
    let h = 0;
    for (let i=0;i<name.length;i++) h = (h*31 + name.charCodeAt(i)) >>> 0;
    const n = (h % 24) + 1;
    return `/avatars/avatar-${n}.webp`;
  };
  const attendeesForPreview = (row.attendees || []).map((a:any, attIdx: number) => {
    const isCandidate = row.linkedCandidate?.name === a.name;
    const isContact = row.linkedContact?.name === a.name;
    const type = isCandidate ? "candidate" : isContact ? "contact" : "";
    const baseId = isCandidate ? row.linkedCandidate?.id : isContact ? row.linkedContact?.id : undefined;
    return {
      name: a.name,
      role: a.name === row.organizer?.name ? "Organizer" : isCandidate ? "Candidate" : isContact ? "Contact" : "Internal attendee",
  ...
  if (row.organizer && !attendeesForPreview.some(a=>a.name===row.organizer.name)) {
    attendeesForPreview.unshift({ name: row.organizer.name, role: "Organizer", avatar: avatarFor(row.organizer.name, row.organizer.avatar), status: firstStatus, type: "", id: undefined });
  }
  // ensure linked candidate/contact are included if missing
  if (row.linkedCandidate && !attendeesForPreview.some(a=>a.name===row.linkedCandidate!.name)) {
    attendeesForPreview.push({ name: row.linkedCandidate.name, role:"Candidate", avatar: avatarFor(row.linkedCandidate.name, row.linkedCandidate.avatar), status: statusFor(attendeesForPreview.length), type:"candidate", id: row.linkedCandidate.id });
  }
  if (row.linkedContact && !attendeesForPreview.some(a=>a.name===row.linkedContact!.name)) {
    attendeesForPreview.push({ name: row.linkedContact.name, role:"Contact", avatar: avatarFor(row.linkedContact.name, row.linkedContact.avatar), status: statusFor(attendeesForPreview.length), type:"contact", id: row.linkedContact.id });
  }
  const descParts = [];
  ...
  const linkedRecords: { type: string; item: any }[] = [];
  if (row.linkedCandidate) linkedRecords.push({ type: "Candidate", item: { id: row.linkedCandidate.id, name: row.linkedCandidate.name, avatar: row.linkedCandidate.avatar ?? "", email: "" } });
  if (row.linkedContact) linkedRecords.push({ type: "Contact", item: { id: row.linkedContact.id, name: row.linkedContact.name, avatar: row.linkedContact.avatar ?? "", email: "" } });
  if (row.linkedOrganization) linkedRecords.push({ type: "Organization", item: { id: row.linkedOrganization.id, name: row.linkedOrganization.name, initials: row.linkedOrganization.initials, relationship: (row.linkedOrganization as any).parent ? "Subsidiary" : "Holding", color: (row.linkedOrganization as any).color, parent: (row.linkedOrganization as any).parent } });
  if (row.linkedJob) linkedRecords.push({ type: "Job", item: { id: row.linkedJob.id, title: row.linkedJob.title } });
  if (row.linkedOpportunity) linkedRecords.push({ type: "Opportunity", item: { id: row.linkedOpportunity.id, title: row.linkedOpportunity.title } });
  return {
    id: numericId,
    date: dateKey,
    hour,
    ...
    endMinute,
    title: row.title,
    eventType: row.meetingType,
    description: descParts.join(" ") || `${row.meetingType} meeting`,
    ...
    eventUrl: row.meetingPlaceHref || row.meetingPlace || `https://app.wiggli.io/meetings/${row.ref}`,
    eventProvider: row.meetingPlaceProvider,
    previewAttendees: attendeesForPreview,
    linkedRecords,
    storedLinkedRecords: linkedRecords,
  } as unknown as EventItem;
}

function LocationTypeCell({ type }: { type: string }) {
...
export function MeetingsTable({ filterEntity, filterId, filterName, extraMeetings = [], onScheduleMeeting, rows: providedRows }: { filterEntity?: "candidate" | "contact" | "job" | "organization" | "opportunity"; filterId?: string; filterName?: string; extraMeetings?: any[]; onScheduleMeeting?: () => void; rows?: MeetingRow[] }) {
  ...
  // For demo, inject filter entity into display so it looks linked to current single
  const rows = useMemo(() => {
  ...
  // Merge extraMeetings (created via drawer) on top
  const allRows = useMemo(() => [...extraMeetings.map((e:any)=> ({
    ref: `MT-${Date.now().toString().slice(-6)}`,
    ...
    meetingType: e.eventType || "Meeting",
    organizer: { name: "Axelle Bastin", avatar: "", color: "#0f766e" },
  ...
  const filteredRows = useMemo(() => {
    ...
    return allRows.filter((row) => {
      const hay = [
        ...
        row.meetingType,
        row.organizer?.name,
        row.linkedCandidate?.name,
        row.linkedContact?.name,
        row.linkedJob?.title,
        row.linkedOpportunity?.title,
        row.linkedOrganization?.name,
        row.linkedOrganization?.parent?.name,
        row.attendees?.map((a:any)=>a.name).join(" "),
        row.locationType,
        row.meetingPlace,
        row.createdOn,
        row.createdBy?.name,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 16, width: "100%", minWidth: 0, overflow: "hidden" }}>
      ...
      <table className="jobs-table meetings-table" style={{ minWidth: "max-content" }}>
        <thead>
          <tr>
            <th className="jobs-table-menu-col" style={{width:80, minWidth:80, maxWidth:80, padding:"0 8px !important"}}><div style={{ display: "flex", alignItems:"center", justifyContent: "center" }}><input type="checkbox" aria-label="Select all" style={{ width: 16, height: 16, accentColor: "#0f766e" }} /></div></th>
        ...
        <tbody>
          {filteredRows.length === 0 ? (
            ...
            <tr key={row.ref} tabIndex={0} onClick={(e) => { if (!(e.target as HTMLElement).closest("input, button, a")) openPreview(row); }} onKeyDown={(e) => { if (e.key === "Enter") openPreview(row); }} style={{ cursor: "pointer" }}>
              ...
              <td>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => router.push(`/contacts/10000627`)}>
              ...
              <td>
                {row.linkedCandidate ? (
                  <span
                    onClick={() => router.push(`/candidates/${row.linkedCandidate!.id}`)}
                    style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: "#1e293b", textDecoration: "none" }} onMouseEnter={(e)=> (e.currentTarget.style.textDecoration="underline")} onMouseLeave={(e)=> (e.currentTarget.style.textDecoration="none")}
                    title={row.linkedCandidate.name}
                  >
              ...
              <td>
                {row.linkedContact ? (
                  <span onClick={() => router.push(`/contacts/${row.linkedContact!.id}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: "#1e293b", textDecoration: "none" }} onMouseEnter={(e)=> (e.currentTarget.style.textDecoration="underline")} onMouseLeave={(e)=> (e.currentTarget.style.textDecoration="none")} title={row.linkedContact.name}>
              ...
              <td>
                {row.linkedJob ? (
                  <span onClick={() => router.push(`/jobs/${row.linkedJob!.id}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: "#1e293b", textDecoration: "none" }} onMouseEnter={(e)=> (e.currentTarget.style.textDecoration="underline")} onMouseLeave={(e)=> (e.currentTarget.style.textDecoration="none")}>
              ...
              <td>
                {row.linkedOpportunity ? (
                  <span onClick={() => router.push(`/opportunities/${row.linkedOpportunity!.id}`)} style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", color: "#1e293b", textDecoration: "none" }} onMouseEnter={(e)=> (e.currentTarget.style.textDecoration="underline")} onMouseLeave={(e)=> (e.currentTarget.style.textDecoration="none")}>
              ...
              <td>
                {row.linkedOrganization ? (
                  <span onClick={() => router.push(`/organizations/${row.linkedOrganization!.id}`)} style={{ display: "inline-flex", alignItems: "center", gap: 4, cursor: "pointer", color: "#1e293b" }} onMouseEnter={(e)=> (e.currentTarget.style.textDecoration="underline")} onMouseLeave={(e)=> (e.currentTarget.style.textDecoration="none")} title={row.linkedOrganization.name}>
